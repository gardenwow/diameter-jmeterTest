...
import api.IMessage;
import api.parser.ParseException;
import junit.framework.Assert;
import org.jdiameter.api.*;
import org.jdiameter.dictionary.AvpDictionary;
import org.jdiameter.dictionary.AvpRepresentation;
import org.jdiameter.impl.helpers.EmptyConfiguration;
import org.jdiameter.impl.helpers.Loggers;
import org.jdiameter.impl.parser.MessageImpl;
import org.jdiameter.impl.parser.MessageParser;
...
//    serverURI = "aaa://" + serverHost + ":" + serverPort;
    ...
    private void initStack()
    {
        ...
        try
        {
            avpDictionary.parseDictionary(this.getClass().getClassLoader().getResourceAsStream("C:\\Users\\sereg\\IdeaProjects\\diameter\\src\\main\\java\\config\\dictionary.xml"));
        ...
        try
        {
            stack = new org.jdiameter.impl.StackImpl();
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    private ArrayList<AvpSet> parseAction(Element action) throws InternalException
    {
        ...
        for(int i = 0; i < commandNodes.getLength(); i++)
        {
            ...
            if (commandNode.getNodeType() == Node.ELEMENT_NODE) {
                // Now we add the AVPs
                IMessage msg = parser.createEmptyMessage(Integer.valueOf(0), 0L);

                addAvps(msg.getAvps(), commandNode.getChildNodes());

                if (msg instanceof Message) { // Проверяем, является ли msg типом Message
                    messages.add(((Message) msg).getAvps()); // Приводим тип IMessage к Message
                }
            }
        }
