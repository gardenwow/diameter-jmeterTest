...
package org.jdiameter.impl;
 ...
 import api.controller.IPeer;
 import api.fsm.EventTypes;
 import api.io.IConnectionListener;
 import api.io.TransportException;
 import org.jdiameter.api.*;
 import org.jdiameter.api.app.StateChangeListener;
 import org.jdiameter.common.concurrent.data.ISessionDatasource;
 import org.jdiameter.common.concurrent.statistic.IStatistic;
 import org.jdiameter.common.concurrent.statistic.IStatisticManager;
 import org.jdiameter.common.concurrent.statistic.IStatisticRecord;
 import org.jdiameter.impl.controller.AbstractPeer;
 import org.jdiameter.impl.helpers.IPConverter;
...
public class MetaDataImpl implements IMetaData {
  ...
  protected class ClientLocalPeer extends AbstractPeer implements IPeer {
    ...
    public URI getUri() {
      try {
        return new URI(stack.getConfiguration().getStringValue(org.jdiameter.impl.helpers.Parameters.OwnDiameterURI.ordinal(), (String) org.jdiameter.impl.helpers.Parameters.OwnDiameterURI.defValue()));
      }
    ...
    public String getRealmName() {
      return stack.getConfiguration().getStringValue(org.jdiameter.impl.helpers.Parameters.OwnRealm.ordinal(), (String) org.jdiameter.impl.helpers.Parameters.OwnRealm.defValue());
    }

    @Override
    public long getVendorId() {
      return stack.getConfiguration().getLongValue(org.jdiameter.impl.helpers.Parameters.OwnVendorID.ordinal(), (Long) org.jdiameter.impl.helpers.Parameters.OwnVendorID.defValue());
    }

    @Override
    public String getProductName() {
      return stack.getConfiguration().getStringValue(org.jdiameter.impl.helpers.Parameters.OwnProductName.ordinal(), (String) org.jdiameter.impl.helpers.Parameters.OwnProductName.defValue());
    }

    @Override
    public long getFirmware() {
      return stack.getConfiguration().getLongValue(org.jdiameter.impl.helpers.Parameters.OwnFirmwareRevision.ordinal(), -1L);
    }
    ...
    public Set<ApplicationId> getCommonApplications() {
      ...
      if (appIds.isEmpty()) {
        Configuration[] apps = stack.getConfiguration().getChildren(org.jdiameter.impl.helpers.Parameters.ApplicationId.ordinal());
        if (apps != null) {
          if (logger.isDebugEnabled()) {
            logger.debug("Stack configuration has apps list size of  [{}]. Looping through them", apps.length);
          }
          for (Configuration a : apps) {
            long vnd = a.getLongValue(org.jdiameter.impl.helpers.Parameters.VendorId.ordinal(), 0L);
            long auth = a.getLongValue(org.jdiameter.impl.helpers.Parameters.AuthApplId.ordinal(), 0L);
            long acc = a.getLongValue(org.jdiameter.impl.helpers.Parameters.AcctApplId.ordinal(), 0L);
            if (logger.isDebugEnabled()) {
              logger.debug("Adding app id vendor [{}] auth [{}] acc [{}]", new Object[]{vnd, auth, acc});
            }
            if (auth != 0) {
              appIds.add(ApplicationId.createByAuthAppId(vnd, auth));
            }
            if (acc != 0) {
              appIds.add(ApplicationId.createByAccAppId(vnd, acc));
            }
          }
        }
        else {
    ...
    public InetAddress[] getIPAddresses() {
      if (addresses.length == 0) {
        String address = stack.getConfiguration().getStringValue(org.jdiameter.impl.helpers.Parameters.OwnIPAddress.ordinal(), null);
