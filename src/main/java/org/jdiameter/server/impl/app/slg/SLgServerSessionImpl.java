...
import org.jdiameter.common.concurrent.app.slg.ISLgMessageFactory;
import org.jdiameter.common.concurrent.app.slg.SLgSessionState;
import org.jdiameter.api.AppAnswerEventImpl;
import org.jdiameter.api.AppRequestEventImpl;
import org.jdiameter.impl.app.slg.SLgSession;
  ...
  public void onTimer(String timerName) {
    if (timerName.equals(SLgSession.TIMER_NAME_MSG_TIMEOUT)) {
      try {
        ...
        try {
          handleEvent(
              new Event(Type.TIMEOUT_EXPIRES, new AppRequestEventImpl(this.sessionData.getBuffer()), null));
        } catch (Exception e) {
      ...
      try {
        switch (request.getCommandCode()) {
          ...
          case ProvideLocationRequest.code:
            handleEvent(
                new Event(Type.RECEIVE_PLR, messageFactory.createProvideLocationRequest(request), null));
            break;
      ...
      try {
        switch (answer.getCommandCode()) {
          ...
          case LocationReportAnswer.code:
            handleEvent(new Event(Type.RECEIVE_LRA, messageFactory.createLocationReportRequest(request),
