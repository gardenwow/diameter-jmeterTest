...
package org.jdiameter.server.impl.app.s6a;
 ...
 import org.jdiameter.common.concurrent.app.s6a.IS6aMessageFactory;
 import org.jdiameter.common.concurrent.app.s6a.S6aSessionState;
 import org.jdiameter.api.AppAnswerEventImpl;
 import org.jdiameter.api.AppRequestEventImpl;
 import org.jdiameter.impl.app.s6a.S6aSession;
...
public class S6aServerSessionImpl extends S6aSession implements ServerS6aSession, EventListener<Request, Answer>, NetworkReqListener {
  ...
  public void sendAuthenticationInformationAnswer(JAuthenticationInformationAnswer answer)
    ...
    send(Type.SEND_MESSAGE, null, answer);
  }
  ...
  public void sendCancelLocationRequest(JCancelLocationRequest request)
      throws InternalException, IllegalDiameterStateException, RouteException, OverloadException {
    send(Type.SEND_MESSAGE, request, null);
  }
  ...
  public boolean handleEvent(StateEvent event) throws InternalException, OverloadException {
    try {
      ...
      Type eventType = (Type) event.getType();
      switch (state) {
  ...
  public void timeoutExpired(Request request) {
    try {
      handleEvent(new Event(Type.TIMEOUT_EXPIRES, new AppRequestEventImpl(request), null));
    }
    catch (Exception e) {
  ...
  protected void send(Type type, AppEvent request, AppEvent answer) throws InternalException {
    try {
      if (type != null) {
  ...
  public void onTimer(String timerName) {
    if (timerName.equals(IDLE_SESSION_TIMER_NAME)) {
      ...
      try {
        sendAndStateLock.lock();
        try {
          handleEvent(new Event(Type.TIMEOUT_EXPIRES, new AppRequestEventImpl(this.sessionData.getBuffer()), null));
        }
        catch (Exception e) {
      ...
      try {
        switch (request.getCommandCode()) {
          case JAuthenticationInformationRequest.code:
            handleEvent(new Event(Type.RECEIVE_AIR, messageFactory.createAuthenticationInformationRequest(request), null));
            break;

          case JPurgeUERequest.code:
            handleEvent(new Event(Type.RECEIVE_PUR, messageFactory.createPurgeUERequest(request), null));
            break;

          case JUpdateLocationRequest.code:
            handleEvent(new Event(Type.RECEIVE_ULR, messageFactory.createUpdateLocationRequest(request), null));
            break;

          case JNotifyRequest.code:
            handleEvent(new Event(Type.RECEIVE_NOR, messageFactory.createNotifyRequest(request), null));
            break;
      ...
      try {
        switch (answer.getCommandCode()) {
          case JCancelLocationAnswer.code:
            handleEvent(new Event(Type.RECEIVE_CLA, messageFactory.createCancelLocationRequest(request),
                messageFactory.createCancelLocationAnswer(answer)));
            break;

          case JInsertSubscriberDataAnswer.code:
            handleEvent(new Event(Type.RECEIVE_IDA, messageFactory.createInsertSubscriberDataRequest(request),
                messageFactory.createInsertSubscriberDataAnswer(answer)));
            break;

          case JDeleteSubscriberDataAnswer.code:
            handleEvent(new Event(Type.RECEIVE_DSA, messageFactory.createDeleteSubscriberDataRequest(request),
                messageFactory.createDeleteSubscriberDataAnswer(answer)));
            break;

          case JResetAnswer.code:
            handleEvent(new Event(Type.RECEIVE_RSA, messageFactory.createResetRequest(request), messageFactory.createResetAnswer(answer)));
            break;
