...
package org.jdiameter.server.impl.app.cxdx;
 ...
 import org.jdiameter.common.concurrent.app.cxdx.CxDxSessionState;
 import org.jdiameter.common.concurrent.app.cxdx.ICxDxMessageFactory;
 import org.jdiameter.api.AppAnswerEventImpl;
 import org.jdiameter.api.AppRequestEventImpl;
 import org.jdiameter.api.cxdx.CxDxSession;
      ...
      try {
        switch (request.getCommandCode()) {
          case JUserAuthorizationRequest.code:
            handleEvent(new Event(Type.RECEIVE_UAR, messageFactory.createUserAuthorizationRequest(request), null));
            break;

          case JServerAssignmentRequest.code:
            handleEvent(new Event(Type.RECEIVE_SAR, messageFactory.createServerAssignmentRequest(request), null));
            break;

          case JMultimediaAuthRequest.code:
            handleEvent(new Event(Type.RECEIVE_MAR, messageFactory.createMultimediaAuthRequest(request), null));
            break;

          case JLocationInfoRequest.code:
            handleEvent(new Event(Type.RECEIVE_LIR, messageFactory.createLocationInfoRequest(request), null));
            break;
      ...
      try {
        switch (answer.getCommandCode()) {
          ...
          case JPushProfileAnswer.code:
            handleEvent(new Event(Type.RECEIVE_PPA, messageFactory.createPushProfileRequest(request), messageFactory.createPushProfileAnswer(answer)));
            break;

          case JRegistrationTerminationAnswer.code:
            handleEvent(new Event(Type.RECEIVE_RTA, messageFactory.createRegistrationTerminationRequest(request),
